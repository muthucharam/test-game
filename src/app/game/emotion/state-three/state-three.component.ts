...
@Component({
  ...
  styleUrls: ['./state-three.component.css']
})
    ...
    GAMEJSON.DATA.data[0].GAMEINFO.forEach(element => {
      ...
      if (element.gameId == "emotion-motion") {
        element.slideData.forEach(elements => {
          if (elements.sliderType == "SelectSadGameSlide") {
                ...
                if (element.itemName == "outline-word") {
                  ...
                  this.game.add.button(element.itemPosition[0], element.itemPosition[1], element.itemName, () => {
                    ...
                    let answervalidate = GAMEDATA.emotion.filter(answer =>
                      answer.sliderType == "SelectSadGameSlide"
                    );
                    ...
                    if (answerlength.length == 1) {
                      this.game.add.button(element.itemPosition[0], element.itemPosition[1], "outline-word-right", () => { this.game.state.start("StateFinalGame"); }, this, 2, 1, 0);
                      
                          
                      
                  
                    } else if (answerlength.length == 0) {
                      this.game.add.button(element.itemPosition[0], element.itemPosition[1], "outline-word-wrong", () => {   this.game.state.start("StateFinalGame");}, this, 2, 1, 0);
