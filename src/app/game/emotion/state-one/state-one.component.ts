...
@Component({
  ...
  styleUrls: ['./state-one.component.css']
})
    ...
    GAMEJSON.DATA.data[0].GAMEINFO.forEach(element => {
      ...
      if (element.gameId == "emotion-motion") {
        element.slideData.forEach(elements => {
          if (elements.sliderType == "SelectPictureGameSlide") {
                ...
                if (element.itemName == "outline-word") {
                  ...
                  this.game.add.button(element.itemPosition[0], element.itemPosition[1], element.itemName, () => {
                    ...
                    if (answerlength.length == 0) {
                      this.game.add.button(element.itemPosition[0], element.itemPosition[1], "outline-word-right", () => {  this.game.state.start("StateTwoGame");
                    }, this, 2, 1, 0);
                      
                          
                  
                    } else if (answerlength.length == 1) {
                      this.game.add.button(element.itemPosition[0], element.itemPosition[1], "outline-word-wrong", () => { this.game.state.start("StateTwoGame");
                    }, this, 2, 1, 0);
