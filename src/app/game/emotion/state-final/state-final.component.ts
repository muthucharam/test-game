...
@Component({
  selector: 'app-state-final',
  templateUrl: './state-final.component.html',
  styleUrls: ['./state-final.component.css']
})
export class StateFinalComponent implements OnInit {
  ...
  console.log(element)
      ...
      if (element.gameId == "emotion-motion") {
        ...
        element.slideData.forEach(elements => {
          ...
          if (elements.sliderType == "gamecompleteSlide") {
    ...
    GAMEJSON.DATA.data[0].GAMEINFO.forEach(gameInfo => {
      if (gameInfo.gameId == "emotion-motion") {
        ...
        gameInfo.slideData.forEach(sliderInfo => {
          ...
          if (sliderInfo.sliderType == "gamecompleteSlide") {
